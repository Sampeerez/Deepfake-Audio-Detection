...
def _hash_config(extractor: FeatureExtractor, option: str) -> str:
    ...
    if option == "3":
        params.update({"n_lfcc": extractor.n_lfcc, "n_lin": extractor.n_linear_filters})
    if option == "4":
        params["wavelet"] = extractor.wavelet_mother
    if option == "6":
        params.update({"n_cqcc": extractor.n_cqcc, "n_bins": extractor.cqcc_n_bins})
...
def evaluate_raw_on_set(
    ...
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    max_samples = int(params.get("raw_max_samples", 4 * sample_rate))
